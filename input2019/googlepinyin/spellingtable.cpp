...
const char* SpellingTable::arrange(size_t *item_size, size_t *spl_num) {
  ...
  if (need_score_) {
    if (kPrintDebug0)
      LOG_L_OUTPUT(LOG_LVL_INFO, "------------Spelling Possiblities--------------\n");
    ...
    if (kPrintDebug0)
      LOG_L_OUTPUT(LOG_LVL_INFO, "-----max psb: %f, min psb: %f\n", max_score, min_score);

    max_score = log(max_score);
    min_score = log(min_score);

    if (kPrintDebug0)
      LOG_L_OUTPUT(LOG_LVL_INFO, "-----max log value: %f, min log value: %f\n",
             max_score, min_score);
    ...
    for (size_t pos = 0; pos < spelling_num_; pos++) {
      ...
      if (kPrintDebug0) {
        LOG_L_OUTPUT(LOG_LVL_INFO, "---pos:%d, %s, psb:%d\n", pos, this_spl_buf,
