...
void DictBuilder::get_top_lemmas() {
  ...
  if (kPrintDebug0) {
    LOG_L_OUTPUT(LOG_LVL_INFO, "\n------Top Lemmas------------------\n");
    for (size_t pos = 0; pos < top_lmas_num_; pos++) {
      LOG_L_OUTPUT(LOG_LVL_INFO, "--%d, idx:%06d, score:%.5f\n", pos, top_lmas_[pos].idx_by_hz,
             top_lmas_[pos].freq);
    }
  }
}
...
size_t DictBuilder::read_raw_dict(const char* fn_raw,
  ...
  LOG_L_OUTPUT(LOG_LVL_INFO, "read succesfully, lemma num: %d\n", lemma_num);
...
bool DictBuilder::build_dict(const char *fn_raw,
  ...
  LOG_L_OUTPUT(LOG_LVL_INFO, "spelling tree construct successfully.\n");
  ...
  if (kPrintDebug0) {
    LOG_L_OUTPUT(LOG_LVL_INFO, "homo_idx_num_eq1_: %d\n", homo_idx_num_eq1_);
    LOG_L_OUTPUT(LOG_LVL_INFO, "homo_idx_num_gt1_: %d\n", homo_idx_num_gt1_);
    LOG_L_OUTPUT(LOG_LVL_INFO, "top_lmas_num_: %d\n", top_lmas_num_);
  }

  free_resource();

  if (kPrintDebug0) {
    LOG_L_OUTPUT(LOG_LVL_INFO, "Building dict succeds\n");
  }
  return dt_success;
}
...
void DictBuilder::stat_print() {
  LOG_L_OUTPUT(LOG_LVL_INFO, "\n------------STAT INFO-------------\n");
  LOG_L_OUTPUT(LOG_LVL_INFO, "[root is layer -1]\n");
  LOG_L_OUTPUT(LOG_LVL_INFO, ".. max_sonbuf_len per layer(from layer 0):\n   ");
  for (size_t i = 0; i < kMaxLemmaSize; i++)
    LOG_L_OUTPUT(LOG_LVL_INFO, "%d, ", max_sonbuf_len_[i]);
  LOG_L_OUTPUT(LOG_LVL_INFO, "-, \n");

  LOG_L_OUTPUT(LOG_LVL_INFO, ".. max_homobuf_len per layer:\n   -, ");
  for (size_t i = 0; i < kMaxLemmaSize; i++)
    LOG_L_OUTPUT(LOG_LVL_INFO, "%d, ", max_homobuf_len_[i]);
  LOG_L_OUTPUT(LOG_LVL_INFO, "\n");

  LOG_L_OUTPUT(LOG_LVL_INFO, ".. total_son_num per layer:\n   ");
  for (size_t i = 0; i < kMaxLemmaSize; i++)
    LOG_L_OUTPUT(LOG_LVL_INFO, "%d, ", total_son_num_[i]);
  LOG_L_OUTPUT(LOG_LVL_INFO, "-, \n");

  LOG_L_OUTPUT(LOG_LVL_INFO, ".. total_node_hasson per layer:\n   1, ");
  for (size_t i = 0; i < kMaxLemmaSize; i++)
    LOG_L_OUTPUT(LOG_LVL_INFO, "%d, ", total_node_hasson_[i]);
  LOG_L_OUTPUT(LOG_LVL_INFO, "\n");

  LOG_L_OUTPUT(LOG_LVL_INFO, ".. total_sonbuf_num per layer:\n   ");
  for (size_t i = 0; i < kMaxLemmaSize; i++)
    LOG_L_OUTPUT(LOG_LVL_INFO, "%d, ", total_sonbuf_num_[i]);
  LOG_L_OUTPUT(LOG_LVL_INFO, "-, \n");

  LOG_L_OUTPUT(LOG_LVL_INFO, ".. total_sonbuf_allnoson per layer:\n   ");
  for (size_t i = 0; i < kMaxLemmaSize; i++)
    LOG_L_OUTPUT(LOG_LVL_INFO, "%d, ", total_sonbuf_allnoson_[i]);
  LOG_L_OUTPUT(LOG_LVL_INFO, "-, \n");

  LOG_L_OUTPUT(LOG_LVL_INFO, ".. total_node_in_sonbuf_allnoson per layer:\n   ");
  for (size_t i = 0; i < kMaxLemmaSize; i++)
    LOG_L_OUTPUT(LOG_LVL_INFO, "%d, ", total_node_in_sonbuf_allnoson_[i]);
  LOG_L_OUTPUT(LOG_LVL_INFO, "-, \n");

  LOG_L_OUTPUT(LOG_LVL_INFO, ".. total_homo_num per layer:\n   0, ");
  for (size_t i = 0; i < kMaxLemmaSize; i++)
    LOG_L_OUTPUT(LOG_LVL_INFO, "%d, ", total_homo_num_[i]);
  LOG_L_OUTPUT(LOG_LVL_INFO, "\n");

  LOG_L_OUTPUT(LOG_LVL_INFO, ".. son buf allocation number with only 1 son: %d\n", sonbufs_num1_);
  LOG_L_OUTPUT(LOG_LVL_INFO, ".. son buf allocation number with more than 1 son: %d\n",
         sonbufs_numgt1_);
  LOG_L_OUTPUT(LOG_LVL_INFO, ".. total lemma node number: %d\n", total_lma_node_num_ + 1);
}
